...
var emoji_picker_template_data = [];
...
exports.generate_emoji_picker_data = function (realm_emojis) {
    ...
    emoji_picker_template_data = categories.map(function (category) {
        return {
            name: category.name,
            icon: category.icon,
...
var generate_emoji_picker_content = function (id) {
    ...
    return templates.render('emoji_popover_content', {
        message_id: id,
        emoji_categories: emoji_picker_template_data,
    });
};
...
function filter_emojis() {
    ...
    if (search_term !== '') {
        var search_results = [];
        _.each(complete_emoji_catalog, function (emojis) {
            _.each(emojis, function (emoji_dict) {
                if (emoji_dict.name.indexOf(search_term) !== -1) {
                    search_results.push(emoji_dict.name);
                }
            });
        });
        var result = [];
...
function may_be_change_focused_emoji(next_coord) {
...
function may_be_change_active_section(next_section) {
    if (next_section >= 0 && next_section < emoji_picker_template_data.length) {
        current_section = next_section;
        current_index = 0;
        var offset = subhead_offsets[current_section];
        if (offset) {
            $(".emoji-popover-emoji-map").scrollTop(offset.position_y);
            may_be_change_focused_emoji({
                section: current_section,
                index: current_index,
            });
        }
...
exports.navigate = function (e, event_name) {
    ...
    if (is_filter_focused && event_name === 'down_arrow') {
        ...
        return may_be_change_focused_emoji(next_coord);
    }
    return false;
};
...
exports.emoji_select_tab = function (elt) {
    ...
    if (scrolltop === 0) {
        currently_selected = subhead_offsets[0].section;
    }
